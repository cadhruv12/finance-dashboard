...
// ═══════════════════════════════════════════════════════════
// P&L — MAIN RENDER (FULL STRUCTURE)
// ═══════════════════════════════════════════════════════════
...
function setupPLComparison() {
    ...
    btn.addEventListener('click', () => {
        ...
        const monA = months[parseInt(mA) - 1]
        const monB = months[parseInt(mB) - 1]

        const rowsA = allPLData.filter(r => r.year == yearA && r.month === monA)
        const rowsB = allPLData.filter(r => r.year == yearB && r.month === monB)
...
function renderPLComparison(plA, plB, labelA, labelB) {
    ...
    let revA = plA.grossSales
    let revB = plB.grossSales
    let gpA  = plA.grossProfit
    let gpB  = plB.grossProfit
    let npA  = plA.netProfit
    let npB  = plB.netProfit
    let opA  = plA.totalOpEx
    let opB  = plB.totalOpEx
    ...
    const setKpi = (idVar, idSub, a, b, label) => {
        ...
        const neg = v < 0
        document.getElementById(idVar).textContent =
            neg ? '(' + formatK(Math.abs(v)) + ')' : formatK(v)
        document.getElementById(idVar).style.color = neg ? '#c0392b' : '#2e7d32'
        document.getElementById(idSub).textContent =
            `${labelA}: ${formatK(a)} · ${labelB}: ${formatK(b)} · ${pct.toFixed(1)}%`
    }
